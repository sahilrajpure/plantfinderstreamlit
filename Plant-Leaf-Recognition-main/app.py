...
def main():
    ...
    if st.button("🌱Similar plant Analyzer with AI"):
        flask_process = subprocess.Popen(["python", "index.py"])  # Runs Flask app in the background
        time.sleep(3)  # Wait for the server to start (adjust if needed)
        webbrowser.open_new_tab("http://127.0.0.1:5000")  # Opens Flask app in a new tab
        st.success("Flask App is running! Opened in a new tab.")
